<div id="calendar_{{$uuid}}" class="calendar-container">
    ...
    <div class="lg:grid lg:grid-cols-12 lg:gap-x-16">
        <div class="mt-10 text-center lg:col-start-8 lg:col-end-13 lg:row-start-1 lg:mt-9 xl:col-start-9">
            ...
            <div class="grid grid-cols-7 gap-px mt-2 text-sm bg-gray-200 rounded-lg shadow isolate ring-1 ring-gray-200 calendar-dates"></div>
            <div class="flex flex-col mt-8 md:flex-row gap-x-2 gap-y-4">
                @if($partner == '')
    ...
    <div class="clearfix participants_popup pro_page_pop_up new_popup" style="z-index: 10; max-height:600px; overflow:auto">
        <div class="clearfix pro_soc_con_face_inner">
            ...
            <div class="pro_pop_body">
                <div class="max-w-md mx-auto event-participants-show sm:max-w-3xl">
                    <div>
                        <div class="text-center">
                            <h2 class="mt-2 text-base font-semibold leading-6 text-gray-900">All participants</h2>
                            <p class="mt-1 text-sm text-gray-500 event-title">Recording session</p>
                        </div>
                    </div>
                    <div class="mt-10">
                        <ul role="list" class="grid grid-cols-1 gap-4 mt-4 participants-large sm:grid-cols-2"></ul>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="clearfix add_event_popup pro_page_pop_up new_popup" style="z-index: 10; max-height:600px; overflow:auto">
        <div class="clearfix pro_soc_con_face_inner">
            <div style="padding: 10px 20px;" class="clearfix soc_con_top_logo">
                <a style="opacity:0;" class="logo8">
                    <img class="pro_soc_top_logo defer_loading" src="" data-src="{{ asset('images/1logo8.png') }}"><div>Platform</div>
            ...
            <div class="pro_pop_body">
                <div class="max-w-md mx-auto sm:max-w-3xl">
                    ...
                    <div class="mt-10 mb-16">
                        <div class="grid grid-cols-1 mt-10 participant-form">
                            <div class="w-full">
                            ...
                            <div class="w-full mt-2">
                                ...
                                <div class="flex mt-2">
                                    <ul role="list" class="w-full mt-4 border-t border-b border-gray-200 divide-y divide-gray-200 all-participants">
                                        ...
                                        <li base-id="{{$contactUser->id}}" class="flex items-center justify-between py-4 space-x-3 each-participant">
                                            <div class="flex items-center flex-1 min-w-0 space-x-3">
                                                <div class="flex-shrink-0">
                                                    <img class="w-10 h-10 rounded-full" src="{{$contactPDetails['image']}}" alt="{{$contactPDetails['name']}}">
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-sm font-medium text-gray-900 truncate participant-name">{{$contactPDetails['name']}}</p>
                                                    <p class="text-sm font-medium text-gray-500 truncate">@ {{$contactUser->username}}</p>
                                                </div>
                                            </div>
                                            <div class="flex-shrink-0">
                                                <button type="button" class="relative rounded-md outline-none cursor-pointer participant-remove hover:text-gray-500">
                                        ...
                                        <li base-id="{{$agentUser->id}}" class="flex items-center justify-between hidden py-4 space-x-3 each-participant">
                                            <div class="flex items-center flex-1 min-w-0 space-x-3">
                                                <div class="flex-shrink-0">
                                                    <img class="w-10 h-10 rounded-full" src="{{$agentPDetails['image']}}" alt="{{$agentPDetails['name']}}">
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-sm font-medium text-gray-900 truncate participant-name">{{$agentPDetails['name']}}</p>
                                                    <p class="text-sm font-medium text-gray-500 truncate">@ {{$agentUser->username}}</p>
                                                </div>
                                            </div>
                                            <div class="flex-shrink-0">
                                                <button type="button" class="relative rounded-md outline-none cursor-pointer participant-remove hover:text-gray-500">
                            ...
                            @endif
                            <button type="button" class="w-24 px-2 py-1 mt-4 ml-auto text-sm font-semibold text-white bg-indigo-600 rounded shadow-sm outline-none add-participant-submit hover:bg-indigo-500">Save</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="event-summary" class="element_sample">
        <li class="relative flex py-6 space-x-6 xl:static">
            <div class="flex-auto">
                <h3 class="pr-10 font-semibold text-gray-600 event-title xl:pr-0"></h3>
                <div class="flex flex-col">
                    ...
                    <div class="relative flex flex-col justify-center w-full mt-2 space-y-3 participants-container">
            ...
            <div class="absolute right-0 top-6 xl:relative xl:right-auto xl:top-auto xl:self-center">
                ...
                <div class="absolute right-0 z-10 hidden mt-2 origin-top-right bg-white rounded-md shadow-lg dropdown-menu w-36 ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-0-button" tabindex="-1">
                    <div class="py-1" role="none">
                        <div class="block px-4 py-2 text-sm text-gray-700 cursor-pointer event-edit hover:bg-gray-100" role="menuitem" tabindex="-1" id="menu-0-item-0">Edit</div>
                        <div class="block px-4 py-2 text-sm text-gray-700 cursor-pointer event-delete hover:bg-gray-100" role="menuitem" tabindex="-1" id="menu-0-item-1">Delete</div>
                    </div>
                </div>
            </div>
        </li>
   ...
   $(document).ready(function () {
        ...
        $('body').undelegate('.participants-small', 'click').delegate('.participants-small', 'click', function(e){
            ...
            parent.find('.participants-small dd').each(function(){
                ...
                $('.event-participants-show .participants-large').append('<li><button type="button" class="flex items-center justify-between w-full p-2 space-x-3 text-left border border-gray-300 rounded-full shadow-sm outline-none group"><span class="flex items-center flex-1 min-w-0 space-x-3"><span class="flex-shrink-0 block"><img class="w-10 h-10 rounded-full" src="'+imageSrc+'" alt=""></span><span class="flex-1 block min-w-0"><span class="block text-sm font-medium text-gray-900 truncate">'+name+'</span><span class="block text-sm font-medium text-gray-500 truncate">@'+username+'</span></span></span></button></li>');
            });
                    ...
                    if (response.success && response.data) {
                        ...
                        response.data.forEach(function(ev) {
                            ...
                            if (show) {
                                ...
                                if (ev.participant_users.length) {
                                    ev.participant_users.forEach(function(part) {
                                        participants += '<dd data-id="'+part.id+'" data-user-id="'+part.user_id+'" data-username="'+part.username+'" data-name="'+part.name+'"><img class="w-6 h-6 rounded-full bg-gray-50 ring-2 ring-white" src="'+part.image+'" alt=""></dd>';
                                    });
